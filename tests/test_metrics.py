...
def test_http_path_label_collapses_unmatched_404s_to_sentinel(monkeypatch, tmp_path):
    """Scanner traffic against random 404 URLs must collapse to <unmatched>.

    Otherwise every probe URL (/.git/config, /wp-login.php, …) becomes a
    distinct `path` label value and Prometheus cardinality explodes.
    """
    monkeypatch.setenv("MYMCP_TOKEN_FILE", str(tmp_path / "tokens.json"))
    monkeypatch.setenv("MYMCP_AUDIT_LOG_DIR", str(tmp_path / "audit"))
    monkeypatch.setenv("MYMCP_METRICS_TOKEN", "test-metrics-token")
    ...
    for probe in probes:
        # Any status is fine; we only care about the metric label.
        client.get(probe)

    body = client.get("/metrics", headers={"Authorization": "Bearer test-metrics-token"}).text
    assert 'path="<unmatched>"' in body
    for probe in probes:
        assert f'path="{probe}"' not in body
