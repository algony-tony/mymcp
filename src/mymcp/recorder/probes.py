...
async def run_bash_probe(
    ...
    *,
    ...
    with _tracer.start_as_current_span("recorder.bash_probe") as span:
        span.set_attribute("probe.timeout_sec", timeout_sec)
        ...
        exit_code = proc.returncode if proc.returncode is not None else -1
        span.set_attribute("probe.exit_code", exit_code)
...
async def run_read_file_probe(
    ...
    *,
    ...
    with _tracer.start_as_current_span("recorder.read_file_probe") as span:
        path = str(input.get("path", ""))
        span.set_attribute("probe.path", path)
        try:
            with open(path, "rb") as f:
                raw = f.read(max_bytes + 1)
            truncated = len(raw) > max_bytes
            text = raw[:max_bytes].decode("utf-8", errors="replace")
            span.set_attribute("probe.truncated", truncated)
