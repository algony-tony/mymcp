...
def parse_sections(text: str) -> tuple[str, list[tuple[str, str]]]:
    ...
    header_block is everything before the first '## ' line (typically the
    '# Title' line plus the metadata block). Each section's body is the
    content between its '## name' line and the next '## ' line (exclusive),
    with surrounding blank lines stripped. Sections are returned in their
    original order so callers can preserve layout when reassembling.
    """
    lines = text.split("\n")
...
def apply_section_updates(
    ...
    *,
    ...
    section names are appended at the end. ``header`` (if not None) replaces
    the pre-first-section block; pass None to keep the existing header.
    Sections not mentioned in ``section_updates`` keep their existing bodies.
    """
    existing_header, existing = parse_sections(current)
    section_map = dict(existing)
    order = [name for name, _ in existing]
    for name, body in section_updates.items():
        if name not in section_map:
            order.append(name)
        section_map[name] = body.strip("\n")
